...
(function () {
    var MENSAJES = {
        comunidad: {
            ...
            texto: 'Una incidencia que parece pequeña puede afectar a todo tu barrio. Reportarla ayuda a que el problema se identifique y se resuelva a tiempo.',
        ...
        habitos: {
            ...
            texto: 'Separar tus residuos es un hábito. Repetido por muchas personas, ese pequeño gesto puede transformar la forma en que toda una comunidad los gestiona.',
        ...
        convivencia: {
            ...
            texto: 'Lo que hacemos con nuestros residuos también comunica cómo convivimos con los demás. Cuidar los espacios comunes construye un entorno mejor para todos.',
        ...
        riesgos: {
            ...
            texto: 'Los residuos mal gestionados no desaparecen: la contaminación y la acumulación pueden terminar afectando el entorno y la calidad de vida de todos.',
        ...
        consumo: {
            ...
            texto: '¿Alguna vez pensaste cuántos residuos genera lo que consumís? La cantidad que producimos está directamente ligada a nuestros hábitos de consumo.',
        ...
        ambiente: {
            ...
            texto: 'Vivir en un ambiente sano no es un lujo, es un derecho de todos. Cada contenedor bien gestionado es un paso hacia ese derecho.',
        ...
        patrimonio: {
            ...
            texto: 'Las plazas, calles y costas que compartimos también son patrimonio de todos. Cuidarlas es una forma de cuidar nuestra identidad colectiva.',
            enlace: 'Ver más sobre este derecho'
        },
        futuras: {
            icono: '🌱',
            ...
            texto: 'Lo que hacemos hoy con nuestros residuos también le pertenece a las próximas generaciones. Gestionar bien hoy es cuidar el mañana.',
            enlace: 'Ver más sobre este derecho'
        }
    };
    var ORDEN = ['comunidad', 'habitos', 'convivencia', 'riesgos', 'consumo', 'ambiente', 'patrimonio', 'futuras'];
    var ROTACION_MS = 12000;
    var CLAVE_CERRADO = 'sigeruAgenteCerrado';

    function href(id) { return 'impacto-social.html#' + id; }
    ...
    function iniciarFlotante(contenedor) {
        ...
        contenedor.className = 'agente-flotante' + (cerrado ? ' agente-flotante--cerrado' : '');
        ...
        function reiniciarBarra() {
            ...
            elBarra.style.animation = 'agenteProgreso ' + ROTACION_MS + 'ms linear forwards';
        }
        ...
        contenedor.querySelector('.agente-flotante__cerrar').addEventListener('click', function () {
            if (timer) clearInterval(timer);
            sessionStorage.setItem(CLAVE_CERRADO, '1');
            contenedor.classList.add('agente-flotante--cerrado');
        });

        contenedor.querySelector('.agente-flotante__lanzador').addEventListener('click', function () {
    ...
    function insertar(idContenedor, idMensaje, opciones) {
        ...
        if (!(opciones && opciones.repetible) && localStorage.getItem(claveDescarte) === '1') {
            contenedor.innerHTML = '';
            return;
        }

        contenedor.innerHTML =
            '<div class="agente-inline" style="--agente-color:' + m.color + '">' +
            '  <div class="agente-inline__icono">' + m.icono + '</div>' +
            '  <div class="agente-inline__cuerpo">' +
            '    <span class="agente-inline__etiqueta">' + m.etiqueta + '</span>' +
            '    <p>' + m.texto + '</p>' +
            '    <a href="' + href(idMensaje) + '" target="_blank" rel="noopener">' + m.enlace + ' →</a>' +
            '  </div>' +
        ...
        contenedor.querySelector('.agente-inline__cerrar').addEventListener('click', function () {
            tarjeta.classList.add('agente-inline--saliendo');
            localStorage.setItem(claveDescarte, '1');
            setTimeout(function () { contenedor.innerHTML = ''; }, 320);
        });
    }
    ...
    window.SiGeRUAgente = { insertar: insertar, MENSAJES: MENSAJES, ORDEN: ORDEN };
})();
