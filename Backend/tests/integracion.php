...
try {
    ...
    $admin = new ClientePrueba();
    $anon = new ClientePrueba();
    $vecino = new ClientePrueba();
    ...
    foreach (['usuarios/usuarios.php', 'gestion/contenedores.php', 'gestion/centros_acopio.php', 'gestion/maquinaria.php', 'recoleccion/camiones.php'] as $ruta) {
        $admin->pedir($ruta);
        $admin->pedir($ruta, 'PATCH', [], 405);
        $admin->pedir($ruta, 'POST', [], 400);
    }
    foreach (['administrador', 'vecino', 'chofer', 'recolector', 'operario'] as $rol) {
        ...
        if ($rol === 'operario') {
            $datos['centro_id'] = 1;
            $datos['especialidad'] = 'Clasificación';
        }
        ...
        $cuenta = new ClientePrueba();
        $cuenta->login($email, 'Prueba!234');
    ...
    $admin->pedir('usuarios/logout.php', 'POST');
    $admin->pedir('usuarios/sesion.php', 'GET', null, 401);
